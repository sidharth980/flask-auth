...
def main():
    ...
    for video_frame_no in range(FPS*seconds):
        ...
        for each in bars:
            ...
            if(bar_count < no_of_available_divisions):
                barTop = screen_height - int(bar_heights[bar_count]) -each.min_height
            else:
    ...
    try:
        ffmpeg.concat(input_video, input_audio, v=1, a=1).output(f'{filename}_finished.mp4').run()
